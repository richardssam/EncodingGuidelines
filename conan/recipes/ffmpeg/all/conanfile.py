...
class FFMpegConan(ConanFile):
    ...
    def _dependencies(self):
        return {
            ...
            "with_libzimg": ["avcodec"],
            "with_libvmaf": ["avfilter"],
            "with_opencolorio": ["avfilter"],
    ...
    def requirements(self):
        ...
        if self.options.get_safe("with_libvmaf"):
            self.requires("libvmaf/3.0.0")
    ...
    def source(self):
        if self.version == "8.1.1":
            # Allow overriding the local source path via environment variable
            local_src = os.environ.get("CONAN_FFMPEG_LOCAL_SOURCE", "Y:/FFmpeg")
            self.output.info(f"Copying FFmpeg source from {local_src}")
            copy(self, "*", src=local_src, dst=self.source_folder, excludes=["config.h", "config_components.h", "config.log", "ffbuild/config.mak", "ffbuild/config.log"])
        else:
    ...
    def _patch_sources(self):
        ...
        if self.options.with_ssl == "openssl":
            ...
            if Version(self.version) >= "8.0":
                ...
                pattern = "check_lib openssl openssl/ssl.h OPENSSL_init_ssl -lssl -lcrypto -lws2_32 -lgdi32 ||"
                if "DTLS_get_data_mtu" in load(self, configure_path):
                     pattern = "check_lib openssl openssl/ssl.h DTLS_get_data_mtu -lssl -lcrypto -lws2_32 -lgdi32 ||"
                
                replace_in_file(self, configure_path, pattern, pattern.replace("-lssl -lcrypto -lws2_32 -lgdi32", openssl_libs))
            else:
                openssl_libs = load(self, os.path.join(self.build_folder, "openssl_libs.list"))
    ...
    def generate(self):
        ...
        args = [
            ...
            opt_enable_disable("libvmaf", self.options.get_safe("with_libvmaf")),
            ...
            opt_enable_disable("libopencolorio", self.options.get_safe("with_opencolorio")),
            opt_enable_disable("libzimg", self.options.get_safe("with_libzimg")),
        ]
    ...
    def package_info(self):
        ...
        # Conan 2.0: Propagate transitive shared libraries to run environment
        for dep in self.dependencies.values():
            if dep.cpp_info.libdirs:
                for libdir in dep.cpp_info.libdirs:
                    self.runenv_info.prepend_path("DYLD_LIBRARY_PATH", libdir)
